...
`claude-extension-authoring` Skillを使うと、対象の種類を選び、frontmatter、権限、参照資料、補助スクリプト、検証方法を含む構成を作成できます。

作成後は`claude-extension-reviewer` Subagentへ対象パスを渡します。このSubagentは作成Skillをfrontmatterの`skills`からプリロードし、構造検証、公式仕様、最小権限、コンテキスト効率、Hookの決定性、根拠とテストを読み取り専用で確認します。

```text
/claude-extension-authoring plugins/example/skills/example

claude-extension-reviewerで plugins/example/skills/example と関連するPlugin設定をレビューしてください。
```
